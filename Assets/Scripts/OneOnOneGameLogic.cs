...
//			ThisLobbyName = PlayerPrefs.GetString ("amount");
			...
			if (PhotonNetwork.IsMasterClient) {
				...
				MasterName.text =""+PlayerPrefs.GetString ("username");
				...
				dpRemote.sprite = ProfilePic [num];
				RemoteText.text =""+PlayerPrefs.GetString ("username");
			}
//			StartCoroutine(WinnerAPI("2","2"));
		}
		...
		void RoomCanBeDiscard(){
			if ( !isQuitingRoom && !isTimeOut && PhotonNetwork.CurrentRoom.PlayerCount == 1 ) {

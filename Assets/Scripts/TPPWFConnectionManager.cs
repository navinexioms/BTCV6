...
//			http://apienjoybtc.exioms.me/api/Balance/bidgamebalance?userid=2&gamesessionid=1&intWalletType=2&dblamt=10000&gametype=2&roomid=2PLDO1&date=27/02/2019
			...
			print("http://apienjoybtc.exioms.me/api/Balance/bidgamebalance?userid="+PlayerPrefs.GetString("userid")+"&gamesessionid="+1+"&intWalletType="+2+"&dblamt="+GameLobbyName+"&gametype="+2+"&roomid="+RandomRoomName+"&date="+System.DateTime.Now.ToString ("yyyy-MM-dd hh-mm-ss"));
			UnityWebRequest www = new UnityWebRequest ("http://apienjoybtc.exioms.me/api/Balance/bidgamebalance?userid="+PlayerPrefs.GetString("userid")+"&gamesessionid="+1+"&intWalletType="+2+"&dblamt="+GameLobbyName+"&gametype="+2+"&roomid="+RandomRoomName+"&date="+System.DateTime.Now.ToString ("yyyy-MM-dd hh-mm-ss"));
			www.chunkedTransfer = false;
			www.downloadHandler = new DownloadHandlerBuffer ();
			yield return www.SendWebRequest ();
			if (www.error != null) {
...
//					SceneManager.LoadScene ("OneOnOneGameBoard");
		...
		//http://apienjoybtc.exioms.me/api/Room/roomcreatebyuser?struserid=2&strgamesessionid=1&intgametype=2&dblamount=100&strroomname=entername
		//Room name inserted successfully
		IEnumerator AddingRoomName()
		{
			UnityWebRequest www =new UnityWebRequest ("http://apienjoybtc.exioms.me/api/Room/roomcreatebyuser?struserid="+ PlayerPrefs.GetString("userid")+"&strgamesessionid=1&intgametype=2&dblamount="+PlayerPrefs.GetString("amount")+"&strroomname="+PlayerPrefs.GetString("roomid"));
			www.chunkedTransfer = false;
			www.downloadHandler = new DownloadHandlerBuffer ();
			yield return www.SendWebRequest ();
			if (www.error != null) {
				...
				JSONNode jn = SimpleJSON.JSONData.Parse (msg);
				msg = jn [0];
				if (msg.Equals ("Roomnameinsertedsuccessfully")) {
